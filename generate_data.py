...
# Kathmandu Metropolitan City has 32 wards
# We generate realistic vulnerability scores based on known characteristics
# Weights: age(0.3) + material(0.3) + fault_distance(0.25) + soil(0.15)

random.seed(42)

# Known high-risk areas from NSET 2015 post-earthquake survey
# (ward number, area name, age_score, material_score, fault_score, soil_score)
ward_data = [
...
# Approximate ward centroids for Kathmandu (lat, lng)
ward_centroids = {
...
def compute_risk(age, material, fault, soil):
    score = (0.30 * age + 0.30 * material + 0.25 * fault + 0.15 * soil)
...
def get_color(score):
    if score >= 8.0: return "#dc2626"   # red
    if score >= 6.5: return "#f97316"   # orange
...
# Build cascade graph edges (which wards are geographically adjacent)
# Based on actual Kathmandu ward adjacency
adjacency = {
...
def simulate_cascade(start_ward, wards_dict, adj, steps=4):
    """
    Propagate failure probability outward from start_ward.
    P(fail) = start_score/10, diminishes with each hop.
    """
    results = {}
    visited = set()
    queue = [(start_ward, 1.0)]  # (ward, probability)
    ...
    while queue:
        ...
        for n in neighbors:
            if n not in visited:
                neighbor_score = wards_dict[n]["score"] / 10
                # cascade prob = current_prob * neighbor_vulnerability * 0.6 decay
                cascade_prob = prob * neighbor_score * 0.6
                queue.append((n, cascade_prob))

    return results

# Build wards dict for fast lookup
wards_dict = {w["ward"]: w for w in wards}

# Pre-compute cascade from top 3 most vulnerable wards
cascade_scenarios = {}
top3 = [w["ward"] for w in wards[:3]]
for w in top3:
    cascade_scenarios[str(w)] = simulate_cascade(w, wards_dict, adjacency)
...
print("Done! risk_data.json generated successfully.")
print(f"\nTop 5 most vulnerable wards:")
for w in wards[:5]:
    print(f"  Ward {w['ward']} ({w['name']}): {w['score']}/10 — {w['level'].upper()}")

print(f"\nStats: {output['stats']['critical_count']} critical, {output['stats']['high_count']} high risk wards")